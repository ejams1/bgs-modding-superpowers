...
export const TOOL_DEFINITIONS = [
  {
    name: "xedit_status",
    description:
      "Returns the daemon's lifecycle state without blocking: not_started, starting, ready, or failed.",
  ...
  },
  {
    name: "xedit_health",
    description:
      "Pings the ready daemon to confirm it is still responsive (catches zombie daemons); otherwise same as xedit_status.",
    inputSchema: { type: "object" as const, properties: {}, additionalProperties: false },
  },
  {
    name: "xedit_dirty",
    description:
      "Returns xEdit's dirty and pending-shutdown-save state without blocking; see xedit-automation skill for the fail-closed fallback rules.",
    inputSchema: { type: "object" as const, properties: {}, additionalProperties: false },
  },
  {
    name: "xedit_flush",
    description:
      "Drains pending shutdown-save renames and waits for the daemon to exit, then clears lifecycle state.",
    inputSchema: {
      type: "object" as const,
      properties: {
        force: {
  ...
  },
  {
    name: "xedit_stop",
    description:
      "Stops the xEdit daemon and clears MCP state; refuses if unsaved/pending unless force=true.",
    inputSchema: {
      type: "object" as const,
      properties: {
        force: {
          type: "boolean" as const,
          description:
            "If true, stop despite unsaved/pending-shutdown state (explicit, audited abandonment). Default false.",
        },
      },
      additionalProperties: false,
    },
  ...
  },
  {
    name: "xedit_session",
    description:
      "Non-blocking: returns session info when ready, auto-starts the daemon if not_started, else current status.",
    inputSchema: { type: "object" as const, properties: {}, additionalProperties: false },
  },
  {
  ...
  },
  {
    ...
    description:
      "Finds a record by {file, formId} OR {editorId, signature?}. Pass exactly one mode; omit the other entirely.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    description:
      "Composite read of a record: full record, winning override, base record, and conflict status.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    description:
      "Conflict-audit verdict (no_conflict/itpo/itm/minor/breaking) plus winning override and referenced_by listing.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    description:
      "Like xedit_inspect_conflicts, plus the r6 child-group conflict block and, if includeReferences=true, the recursive reference tree.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    // records at scale").
    description:
      "Filters records by parentFormId, signatures, or EditorID/name regex or wildcard; at least one predicate required.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    description:
      "Resolves the ancestor chain (CELL>WRLD, DIAL>INFO, QUST sub-tree, etc.) for a record. Pass {file, formId} OR {editorId, signature?}.",
    inputSchema: {
      type: "object" as const,
      properties: {
        file: {
  ...
  },
  {
    ...
    description:
      "Atomic passthrough for native daemon commands with no dedicated intent tool; still runs the full validation/audit pipeline.",
    inputSchema: {
      type: "object" as const,
      properties: {
        command: {
