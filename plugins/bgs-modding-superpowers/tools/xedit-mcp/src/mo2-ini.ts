...
export function readMo2GamePath(moRoot: string, iniText?: string): string | undefined {
  let text = iniText;
  if (text === undefined) {
    try {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (eq <= 0 || line.slice(0, eq).trim() !== "gamePath") continue;
 ...
 */
export function resolveMo2DataPath(moRoot: string | undefined): string | undefined {
  if (!moRoot) return undefined;
