...
const handler = {
    ...
    async buildPlan(args, ctx) {
        ...
        const targetPath = join(modsDir, args.name);
    ...
    async applyMutation(plan, ctx) {
        ...
        if (plan.args.adopt_existing === true)
        ...
        if (plan.args.adopt_existing === true && result.adopted !== true && result.created !== true) {
            throw new BrokerEnrichedError({
                ...
                message: `mods.create for "${plan.args.name}" returned neither adopted nor created after requesting adopt_existing=true. The deployed control-plane broker may predate adopt_existing support and silently ignored it, which risks MO2's blocking "Mod Exists" dialog. Redeploy the control plane (scripts/install-mo2-control-plane.ps1) and retry.`,
        ...
        await logApplyEvent(handler.toolName, `${adopted ? "adopted" : "created"} "${plan.args.name}" wins_over="${winsOver ?? "none"}" → priority ${reportedPriority}`, bound, plan.planId, profile);
...
registerTool({
    name: "mo2_create_mod",
    tier: "T3",
    description: "Create empty mod via broker mods.create. Optional 'wins_over' positions it just above a named mod in precedence (= just below visually in MO2 GUI). If the mod folder already exists on disk but is unregistered, this refuses at plan time unless 'adopt_existing' is true, in which case it registers the folder as-is (never triggers MO2's modal 'Mod Exists' prompt) and the result includes an 'inventory' (file_count, total_bytes, plugin_names) of whatever the stale folder actually contained -- adopt does not produce an empty mod.",
    inputSchema,
